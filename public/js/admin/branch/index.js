$(document).ready(function () {
    ...
    var table = $('#table-data').DataTable( {
        ...
        "columns": [
            ...
            },
            {
                ...
                render:function(data, type, row )
                {
                    var $inpUrlShow = $('#inp-url-is-matrix');
                    if ($inpUrlShow.length === 0) {
                        return '';
                    }
                    ...
                    var url = $inpUrlShow.val();
                    url = url.replace('FAKE_ID', data.id);
                    var active = data.is_matrix ? 'on' : 'off';
                    return "<a href='"+url+"' class='is-matrix-btn' style='color: #2B6699'><span class='fas fa-toggle-"+active+"''></span></a>";
                },
                "targets": -1
            },
        ],
    ...
    $(document).on('click', '.active-btn', function (e) {
        ...
        Swal.fire({
            title: '¿Desea '+option+' a la sucursal?',
    ...
    $(document).on('click', '.is-matrix-btn', function (e) {
        e.preventDefault();
        var url = $(this).attr('href');
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                {
                    Swal.fire(
                        'Nueva matriz seleccionada',
                        'La sucursal fue elegida como matriz',
                        'success'
                    );
                    table.ajax.reload();
                }
